...
| MCP server registration | `~/.claude/.mcp.json` (global, one entry: `mcpServers.sill`) | `~/.codex/config.toml` (global, `[mcp_servers.sill]` + `[features].hooks = true`) | `install.sh` step 7 writes both |
| Hook wiring | `<project>/.claude/settings.local.json` (merges idempotently on every `--hooks-for` re-run) | `<project>/.codex/hooks.json` (written once; **skipped** if it already exists — a known upgrade gap, see `docs/hooks.md`'s "Adding hooks to a project") | `install.sh` step 8, from `plugin/codex.hooks.json.template` |
